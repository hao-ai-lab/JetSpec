...
from ptd.tree import get_algorithm, build_from_topk
...
def _reference_crossproduct_tree(root_token, topk_tokens_cpu, topk_logprobs_cpu, budget):
...
def test_build_from_topk_accepts_list_inputs():
    ...
    topk_tok, topk_lp = _topk_from_dense(dense)
    t_tensor = build_from_topk("crossproduct", 7, topk_tok, topk_lp, BUDGET, DEV, tree_width=WIDTH)
    t_list = build_from_topk("crossproduct", 7, topk_tok.tolist(), topk_lp.tolist(), BUDGET, DEV, tree_width=WIDTH)
...
def test_crossproduct_build_matches_reference_across_random_logits():
    """CrossProduct.build preserves the exact FIFO heap tree over budgets and widths."""
    root = 7
    block_size = 16
    vocab_size = 200
    ...
    for seed in range(20):
        ...
        for budget in (15, 63, 127):
            for width in (2, 7):
                ...
                exp_tokens, exp_parents, exp_depths, exp_num_nodes = _reference_crossproduct_tree(
                    root,
                    topk_tok.tolist(),
                    topk_lp.tolist(),
                    budget,
                )
                tree = get_algorithm("crossproduct").build(
                    root,
                    dense,
                    block_size,
                    width,
