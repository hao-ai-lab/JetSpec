"""JetSpec N2a gate: the multi-sequence PagedKVCache (shared pool + block
...
Mirrors `tests/inference_engine/test_jetspec_engine.py`'s style (per-test seeding, dense reference
tensors, `torch.equal` bitwise checks).
"""
import torch
import pytest

from jetspec.inference_engine.paged_kv_cache import (
    PagedKVCache,
    EvictionRequired,
    EvictionFailed,
)
