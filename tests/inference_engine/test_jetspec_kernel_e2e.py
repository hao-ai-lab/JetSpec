"""JetSpec N3 end-to-end gate: the paged tree-attention triton kernel, wired into
`JetSpecEngine` behind `attn_backend="triton_paged_tree"`, must produce the SAME tokens
...
def _tiny_jetspec(model, attn_backend: str, block_size: int = 16) -> JetSpecEngine:
    """Wire `model` into a `JetSpecEngine` with the chosen backend (no network).
    ...
    if attn_backend == "triton_paged_tree":
        ...
        model.config._attn_implementation = "jetspec_paged_tree"
    else:
...
def _both(make_engine, run):
    ...
    model = make_engine
    sdpa = run(_tiny_jetspec(model, "sdpa"))
    kern = run(_tiny_jetspec(model, "triton_paged_tree"))
    return sdpa, kern
...
def test_n0_generate_block_sizes(block_size):
    ...
    model = _tiny_model(0)
    sdpa = _tiny_jetspec(model, "sdpa", block_size).generate(PROMPT, SP)["token_ids"]
    kern = _tiny_jetspec(model, "triton_paged_tree", block_size).generate(PROMPT, SP)["token_ids"]
...
def test_n2a_generate_batch_kernel_matches_sdpa(seed):
    ...
    every sequence."""
    model = _tiny_model(seed)
    sdpa = _tiny_jetspec(model, "sdpa").generate_batch(PROMPTS, SP)
    kern = _tiny_jetspec(model, "triton_paged_tree").generate_batch(PROMPTS, SP)
...
def test_n2a_generate_batch_block_sizes(block_size):
    model = _tiny_model(0)
    sdpa = _tiny_jetspec(model, "sdpa", block_size).generate_batch(PROMPTS, SP)
    kern = _tiny_jetspec(model, "triton_paged_tree", block_size).generate_batch(PROMPTS, SP)
    for i in range(len(PROMPTS)):
        assert kern[i]["token_ids"] == sdpa[i]["token_ids"], (
