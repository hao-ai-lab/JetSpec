"""JetSpec N2b gate: batched per-sequence TREE-spec decode over the shared
multi-seq paged cache must be token-identical to running single-stream
`JetSpecEngine.generate_tree` on each prompt alone.
...
per-seq KV-routing arithmetic directly. Mirrors `tests/inference_engine/test_jetspec_tree.py`'s and
`tests/inference_engine/test_jetspec_batch.py`'s `_tiny_jetspec` harness. (On b200 in bf16 a batched
...
def test_tree_batch_matches_single_stream_random():
    ...
    for seed in (0, 1, 7):
        ...
        ref = [_single_tree(_tiny_jetspec(model), p, drafter, seed=100 + seed)
               for p in PROMPTS]
        batched = _batch_tree(_tiny_jetspec(model), PROMPTS, drafter, seed=100 + seed)
...
def test_tree_batch_matches_single_stream_echo():
    """Echo drafter: the accum_logp top-1 path IS the greedy chain, so every seq
    ...
    ref = [_single_tree(_tiny_jetspec(model), p, drafter) for p in PROMPTS]
    batched = _batch_tree(_tiny_jetspec(model), PROMPTS, drafter)
    for i in range(len(PROMPTS)):
        assert batched[i]["token_ids"] == ref[i], (
...
def test_tree_batch_block_sizes_match_single_stream():
    ...
    for seed in (0, 1):
        ...
        for block_size in (2, 4, 5):
            drafter = RandomTreeDrafter(128)
            ref = [_single_tree(_tiny_jetspec(model), p, drafter, seed=7, block_size=block_size)
                   for p in PROMPTS]
            batched = _batch_tree(_tiny_jetspec(model), PROMPTS, drafter, seed=7,
...
def test_tree_batch_cache_block_sizes_match_single_stream():
    ...
    for cache_block in (16, 4, 5):
        ref = [_single_tree(_tiny_jetspec(model, cache_block), p, drafter, seed=3)
               for p in PROMPTS]
        torch.manual_seed(3)
        batched = _tiny_jetspec(model, cache_block).generate_tree_batch(
...
def test_tree_batch_order_invariant():
    ...
    model = _tiny_model(3)
    drafter = TargetEchoTreeDrafter(model)
    forward = _tiny_jetspec(model).generate_tree_batch(
        PROMPTS, drafter, block_size=4, tree_width=2, budget=15, sampling_params=SP)
    reversed_prompts = list(reversed(PROMPTS))
    backward = _tiny_jetspec(model).generate_tree_batch(
...
def test_tree_batch_singleton_matches_generate_tree():
    ...
    model = _tiny_model(1)
    drafter = TargetEchoTreeDrafter(model)
    ref = _single_tree(_tiny_jetspec(model), PROMPTS[0], drafter)
    got = _batch_tree(_tiny_jetspec(model), [PROMPTS[0]], drafter)[0]["token_ids"]
    assert got == ref
