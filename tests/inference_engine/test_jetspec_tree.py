"""JetSpec N1 gate: single-stream TREE-spec decode over the paged KV cache must
...
arithmetic directly. Mirrors `tests/inference_engine/test_jetspec_engine.py`'s `_tiny_jetspec` and
`tests/core/test_tree_kv_cache.py`'s fixtures. (On b200 in bf16 a block forward vs the
...
from jetspec.tree import DraftTree
from jetspec.tree._core.base import TreeAlgorithm
...
def _tiny_jetspec(model, block_size: int = 16) -> JetSpecEngine:
    """Wire the same model into a `JetSpecEngine` without touching the network."""
...
def _jetspec_tree(eng, drafter, *, seed=1, return_stats=False):
...
def test_jetspec_tree_lossless_random():
    ...
    model = _tiny_model(0)
    greedy = _greedy(_tiny_jetspec(model))
    jetspec = _jetspec_tree(_tiny_jetspec(model), RandomTreeDrafter(128))["token_ids"]
    ref = _ref_tree(_tiny_llm(model), RandomTreeDrafter(128))["token_ids"]
    n = min(len(greedy), len(jetspec))
    assert ref[:n] == greedy[:n], "DynamicCache tree diverged from greedy"
    assert jetspec[:n] == greedy[:n], "paged-cache tree diverged from greedy (gather bug)"
    assert jetspec == ref, "paged-cache tree != DynamicCache tree (not a drop-in)"


def test_jetspec_tree_lossless_echo():
    ...
    model = _tiny_model(0)
    greedy = _greedy(_tiny_jetspec(model))
    jetspec = _jetspec_tree(_tiny_jetspec(model), TargetEchoTreeDrafter(model))
    ref = _ref_tree(_tiny_llm(model), TargetEchoTreeDrafter(model))
    n = min(len(greedy), len(jetspec["token_ids"]))
    assert ref["token_ids"][:n] == greedy[:n], "DynamicCache tree (echo) diverged from greedy"
    assert jetspec["token_ids"][:n] == greedy[:n], "paged-cache tree (echo) diverged from greedy"
    assert jetspec["token_ids"] == ref["token_ids"], "paged-cache tree (echo) != DynamicCache ref"
    assert jetspec["tpf"] >= 2.0, f"echo should accept multiple tokens/round, got tpf={jetspec['tpf']:.2f}"


def test_jetspec_tree_block_sizes_match_ref():
    ...
    for seed in (0, 1, 7):
        ...
        for block_size in (16, 4, 5):
            jetspec = _jetspec_tree(_tiny_jetspec(model, block_size), RandomTreeDrafter(128))["token_ids"]
            assert jetspec == ref, (
                f"paged tree diverged from DynamicCache ref (seed={seed}, block_size={block_size})"
            )


def test_jetspec_tree_stats_shape():
    ...
    model = _tiny_model(0)
    full = _jetspec_tree(_tiny_jetspec(model), RandomTreeDrafter(128), return_stats=True)
...
def test_jetspec_tree_lossless_deep_tree_beyond_block_depth():
    ...
    compiled buckets.
    """
    from jetspec.inference_engine.engine import _bucket_for_n
    ...
    greedy = _tiny_jetspec(model).generate(PROMPT, sp)["token_ids"]
    out = _tiny_jetspec(model, block_size=4).generate_tree(
        PROMPT,
        drafter,
        block_size=4,
        tree_width=2,
...
def test_jetspec_tree_long_decode_eos_midblock_matches_ref():
    ...
    for tree_block_size in (4, 16):
        model = _tiny_model(0)
        jetspec_full = _long_echo_tree(
            _tiny_jetspec(model), model, tree_block_size=tree_block_size, return_stats=True
        )
        ref_full = _long_echo_tree(
            _tiny_llm(model), model, tree_block_size=tree_block_size, return_stats=True
        )
        assert jetspec_full["token_ids"] == ref_full["token_ids"]
        assert jetspec_full["rounds"] >= 3

        eos_token = _first_unique_midblock_token(jetspec_full["token_ids"], tree_block_size)
        jetspec = _tiny_jetspec(model)
        ref = _tiny_llm(model)
        jetspec.eos_token_ids = {eos_token}
        ref.eos_token_ids = {eos_token}

        jetspec_eos = _long_echo_tree(
            jetspec, model, tree_block_size=tree_block_size, return_stats=True
        )
        ref_eos = _long_echo_tree(
            ref, model, tree_block_size=tree_block_size, return_stats=True
        )

        assert jetspec_eos["token_ids"] == ref_eos["token_ids"]
        assert jetspec_eos["token_ids"][-1] == eos_token
        eos_idx = len(jetspec_eos["token_ids"]) - 1
