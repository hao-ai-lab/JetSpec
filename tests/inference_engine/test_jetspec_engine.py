"""JetSpec N0 gate: a paged KV cache + single-stream AR engine that is
token-identical to `jetspec.core` on a tiny fp32 Qwen3.
...
# --- JetSpecEngine lossless gate (token-identical to jetspec.core) ----------------

def test_jetspec_ar_matches_llm_greedy_fp32():
    """JetSpecEngine greedy == LLM greedy, token-for-token, across seeds and block
    ...
    for seed in (0, 1, 7):
        ...
        for block_size in (16, 4, 5):
            got = _tiny_jetspec(model, block_size).generate(PROMPT, SP)["token_ids"]
            assert got == ref, (
                f"JetSpec AR diverged from jetspec.core (seed={seed}, block_size={block_size})"
            )
        assert len(ref) == SP.max_new_tokens


def test_jetspec_ar_cache_reuse_grows_by_one():
...
def test_jetspec_ar_temperature_sampling_deterministic():
    ...
    ref = _tiny_llm(model).generate(PROMPT, sp)["token_ids"]
    torch.manual_seed(123)
    got = _tiny_jetspec(model).generate(PROMPT, sp)["token_ids"]
    assert got == ref
