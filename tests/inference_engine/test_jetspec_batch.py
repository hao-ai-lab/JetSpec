"""JetSpec N2a gate: continuous-batched AR over the shared multi-seq paged
cache must be token-identical to running single-stream `JetSpecEngine.generate` on
each prompt alone.
...
per-sequence. Mirrors `tests/inference_engine/test_jetspec_engine.py`'s `_tiny_jetspec` harness.
...
def test_batch_matches_single_stream_each_seq_fp32():
    ...
    for seed in (0, 1, 7):
        ...
        ref = [_tiny_jetspec(model).generate(p, SP)["token_ids"] for p in PROMPTS]
        for block_size in (16, 4, 5):
            batched = _tiny_jetspec(model, block_size).generate_batch(PROMPTS, SP)
...
def test_batch_order_invariant():
    ...
    model = _tiny_model(3)
    forward = _tiny_jetspec(model).generate_batch(PROMPTS, SP)
    reversed_prompts = list(reversed(PROMPTS))
    backward = _tiny_jetspec(model).generate_batch(reversed_prompts, SP)
...
def test_batch_singleton_matches_generate():
    ...
    N2a == N0)."""
    model = _tiny_model(1)
    ref = _tiny_jetspec(model).generate(PROMPTS[0], SP)["token_ids"]
    got = _tiny_jetspec(model).generate_batch([PROMPTS[0]], SP)[0]["token_ids"]
    assert got == ref
