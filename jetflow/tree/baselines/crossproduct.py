"""V0 baseline: cross-product tree from independent top-k per depth.
...
the handoff doc — V1-V12 should reduce to crossproduct when their knobs
...
from jetflow.tree._core.ancestor import _build_ancestor_matrix_np, _build_packed_ancestor_matrix_np
from jetflow.tree._core.accept import _build_child_maps_cpu
...
class CrossProduct(TreeAlgorithm):
    ...
    so the cross-product simplification makes the per-depth top-k tensor
