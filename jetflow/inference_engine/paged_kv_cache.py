...
`jetflow.core.llm._select_kv_cache`. N1 builds the tree-spec loop on it.
...
class PagedKVCache(Cache):
    ...
    def __init__(
        ...
        # the registered attention fn reads `_jetflow_attn_meta` (seq order + qq_bias)
        ...
        self._jetflow_attn_meta: Optional[dict] = None
    ...
    def gather(self, positions: torch.Tensor, seq_id: Optional[int] = None) -> None:
        ...
        `jetflow.core.llm._select_kv_cache`. `seq_id` defaults to seq 0."""
        seq_id = self._default_seq_id if seq_id is None else seq_id
