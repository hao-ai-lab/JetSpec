...
The owned-substrate analogue of `jetflow.core.llm.LLM.generate()`: plain
...
# Optional commit-block timer (env JETFLOW_TIME_COMMIT=1): isolates the logical-commit +
...
_TIME_COMMIT = bool(os.environ.get("JETFLOW_TIME_COMMIT"))
_COMMIT_MS = [0.0]
...
def _maybe_extend_tree(tree, draft_logits, tree_drafter, committed, target_hidden,
    ...
    """
    # tree contract (engine -> tree, one-way): public jetflow.tree API only.
    from jetflow.tree import should_extend, splice_extension
...
class JetFlowEngine:
    def __init__(
        ...
        self.resolved_attn_implementation = getattr(
            self.model, "_jetflow_attn_implementation", attn_implementation
        )
        ...
        if torch_compile:
            ...
            self.model._jetflow_attn_implementation = self.resolved_attn_implementation
        ...
        if attn_backend == "triton_paged_tree":
            ...
            self.model.config._attn_implementation = "jetflow_paged_tree"
    ...
    def _get_graphed_tree_accept(self, num_nodes, max_depth, score_dtype):
        from jetflow.inference_engine.graph_capture import GraphedTreeAccept
    ...
    def generate_tree(self, prompt, tree_drafter, block_size: int = 4, tree_width: int = 2,
        ...
        The owned-substrate analogue of `jetflow.core.llm.LLM._generate_tree_kv_cached`:
        ...
        while generated_len < sp.max_new_tokens:
            ...
            if kernel:
                ...
                if compiled:
                    ...
                    # against in tests).
                    cache._handoff_seq_ids = [0]
                    cache._jetflow_attn_meta = {"seq_ids": [0], "qq_bias": qq_bias}
            ...
            if sp.temperature == 0.0:
                ...
                from jetflow.tree._core.accept import gpu_tree_accept
                ...
                if logical_kv and getattr(self, "_use_cudagraph", False) \
                        ...
                        and _env_flag_enabled("JETFLOW_GRAPH_ACCEPT", True):
    ...
    def _batched_decode_forward_kernel(self, cache: PagedKVCache, seq_ids: list,
        ...
        cache._jetflow_attn_meta = {"seq_ids": list(seq_ids), "qq_bias": None}
        input_ids = torch.tensor([[t] for t in tokens], device=self.device)
        position_ids = torch.tensor([[s] for s in seq_lens], device=self.device)
