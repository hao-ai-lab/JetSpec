...
BUDGETS = [int(b) for b in os.environ.get("JETSPEC_SWEEP_BUDGETS", "15,63,127").split(",")]
WIDTH = int(os.environ.get("JETSPEC_SWEEP_WIDTH", "7"))
MAX_NEW = int(os.environ.get("JETSPEC_SWEEP_MAXNEW", "128"))
...
def main():
    if not DRAFT_HEAD:
        raise SystemExit("set JETSPEC_DRAFT_HEAD to a trained DFlash head checkpoint")
    llm = LLM(MODEL)
