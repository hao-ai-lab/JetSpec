"""Block-paged KV cache (JetSpec N0 -> N2a).
...
`jetspec.core.llm._select_kv_cache`. N1 builds the tree-spec loop on it.
...
class PagedKVCache(Cache):
    ...
    def __init__(
        ...
        # the registered attention fn reads `_jetspec_attn_meta` (seq order + qq_bias)
        ...
        self._jetspec_attn_meta: Optional[dict] = None
    ...
    def gather(self, positions: torch.Tensor, seq_id: Optional[int] = None) -> None:
        ...
        `jetspec.core.llm._select_kv_cache`. `seq_id` defaults to seq 0."""
        seq_id = self._default_seq_id if seq_id is None else seq_id
    ...
    def reorder_cache(self, beam_idx: torch.LongTensor) -> None:
        """Beam reorder — JetSpec is greedy; raise to catch misuse."""
    ...
    def get_mask_sizes(self, cache_position: torch.Tensor | int, layer_idx: int) -> tuple:
        ...
        query_length = (
            int(cache_position) if isinstance(cache_position, int)
