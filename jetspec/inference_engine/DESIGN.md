# JetSpec — design + milestone ladder

An owned, minimal high-throughput engine substrate, sitting beside `jetspec/core`
(the HF/SDPA reference). Same one-way contract — it consumes `jetspec.tree` and never
...
`jetspec/core` (HF + `DynamicCache` + SDPA 4D mask) is for **clarity, single-clone
...
(measured: `jetspec_accum_logp` ≡ the fork's native tree, **7.55× decode**) —
but it's an external, heavy dependency we don't own. `JetSpec` is the **owned**
...
big-engine performance, with the JetSpec tree path kept local and lossless.

> Target to beat / approach: the fork's **7.55× decode_cuda_speedup** (gsm8k, B=255,
> Qwen3-8B + epoch6 head). JetSpec reaches parity: **7.31×** verify-only, cudagraph backend.

## What it reuses (don't rebuild)

- **Tree method** — `jetspec.tree.build_from_topk(name, root, topk_tokens, topk_logprobs,
...
- **Verify accept** — `jetspec.tree.tree_accept`, ancestor mask via
  `jetspec.tree.build_ancestor_matrix`.
- **The persistent-cache verify pattern** — `jetspec/core/llm.py
  ...
  a linear prefix. JetSpec's paged cache generalises exactly this gather to block
  storage. Port the *logic*, swap the storage.

## The distinguisher vs `jetspec/core`

| | `jetspec/core` | `JetSpec` |
|---|---|---|
...
## Milestone ladder (mirrors how `jetspec/core` was built: M0→M1→M2)
...
- **N0 — single-stream AR over a paged KV cache.** A `PagedKVCache` (block pool +
  ...
  loop that reproduces `jetspec/core`'s plain `generate()` token-for-token. Gate:
  byte-identical greedy vs `jetspec/core` on CPU (tiny fp32 model) + b200.
- **N1 — single-stream tree spec on the paged cache.** Build via `build_from_topk`,
  ...
  lossless == greedy; accept_len matches `jetspec/core`'s `_generate_tree_kv_cached`.
...
· `b6158d5` triton kernel · `bdc665e` engine integration). JetSpec does paged,
...
8e-3) and token-identical end-to-end (`test_jetspec_kernel_e2e.py`, 13/13). It **reaches
...
1. **Kernel == SDPA** on random inputs (b200) — the correctness gate. Then the full
   JetSpec suite (`test_jetspec_*`) must stay green with the kernel enabled (reuses the
...
**Caveats / constraints:**
- **GPU-bound.** Triton kernels can't be CPU-validated like the rest of JetSpec — N3
...
```
jetspec/inference_engine/
  paged_kv_cache.py   # block pool + block table; allocate/append/gather/free
  engine.py           # JetSpecEngine: prefill + decode loop (consumes jetspec.tree)
