"""JetSpecEngine — single-stream AR decode over a paged KV cache (JetSpec N0).

The owned-substrate analogue of `jetspec.core.llm.LLM.generate()`: plain
...
from jetspec.models.qwen3 import load_target
...
from jetspec.inference_engine.scheduler import Scheduler, SequenceRequest
...
# Optional commit-block timer (env JETSPEC_TIME_COMMIT=1): isolates the logical-commit +
...
_TIME_COMMIT = bool(os.environ.get("JETSPEC_TIME_COMMIT"))
_COMMIT_MS = [0.0]
...
def _maybe_extend_tree(tree, draft_logits, tree_drafter, committed, target_hidden,
    ...
    """
    # tree contract (engine -> tree, one-way): public jetspec.tree API only.
    from jetspec.tree import should_extend, splice_extension
...
class JetSpecEngine:
    def __init__(
        ...
        self.resolved_attn_implementation = getattr(
            self.model, "_jetspec_attn_implementation", attn_implementation
        )
        ...
        if torch_compile:
            ...
            self.model._jetspec_attn_implementation = self.resolved_attn_implementation
        ...
        self.fuse_gemms = bool(fuse_gemms) or _env_flag("JETSPEC_FUSE_GEMMS")
        if attn_backend == "triton_paged_tree":
            from jetspec.inference_engine.paged_attn_backend import register_jetspec_paged_tree

            register_jetspec_paged_tree()
            self.model.config._attn_implementation = "jetspec_paged_tree"
    ...
    def generate(self, prompt, sampling_params: SamplingParams = None) -> dict:
        ...
        if kernel:
            ...
            cache._jetspec_attn_meta = {"seq_ids": [0], "qq_bias": None}
    ...
    def _get_graphed_tree_accept(self, num_nodes, max_depth, score_dtype):
        from jetspec.inference_engine.graph_capture import GraphedTreeAccept
    ...
    def generate_tree(self, prompt, tree_drafter, block_size: int = 4, tree_width: int = 2,
        ...
        # tree contract (engine -> tree, one-way): import only the public jetspec.tree API
        ...
        while generated_len < sp.max_new_tokens:
            ...
            if kernel:
                ...
                if compiled:
                    ...
                    # against in tests).
                    cache._handoff_seq_ids = [0]
                    cache._jetspec_attn_meta = {"seq_ids": [0], "qq_bias": qq_bias}
            ...
            if sp.temperature == 0.0:
                ...
                from jetspec.tree._core.accept import gpu_tree_accept
                ...
                if logical_kv and getattr(self, "_use_cudagraph", False) \
                        ...
                        and _env_flag_enabled("JETSPEC_GRAPH_ACCEPT", True):
    ...
    def generate_batch(self, prompts: list, sampling_params: SamplingParams = None) -> list:
        ...
        cache (JetSpec N2a). Returns a list of `{token_ids, text}` aligned to
    ...
    def _batched_decode_forward_kernel(self, cache: PagedKVCache, seq_ids: list,
        ...
        cache._jetspec_attn_meta = {"seq_ids": list(seq_ids), "qq_bias": None}
        input_ids = torch.tensor([[t] for t in tokens], device=self.device)
        position_ids = torch.tensor([[s] for s in seq_lens], device=self.device)
    ...
    def generate_tree_batch(self, prompts: list, tree_drafter, block_size: int = 4,
        ...
        cache (JetSpec N2b). Returns a list of `{token_ids, text, tpf}` aligned to
