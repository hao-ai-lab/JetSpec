...
triton kernel (JetSpec N3, opt-in).
...
from jetspec.inference_engine.paged_kv_cache import PagedHandle
from jetspec.inference_engine.paged_tree_attn_op import paged_tree_attn


def _jetspec_paged_tree_attn_forward(
    module,
    ...
    **kwargs,
    ...
    meta = cache._jetspec_attn_meta        # {"seq_ids": [...], "qq_bias": tensor | None}
    if meta is None:                   # handoff on but meta unset -> clear error, not a TypeError
        raise RuntimeError(
            "jetspec_paged_tree: paged-handoff active but _jetspec_attn_meta is unset "
            "(the engine seam must set cache._jetspec_attn_meta before the forward)"
        )
...
def register_jetspec_paged_tree() -> None:
    """Register the paged tree-attention interface under `"jetspec_paged_tree"` (idempotent).
    ...
    AttentionInterface().register("jetspec_paged_tree", _jetspec_paged_tree_attn_forward)
    _REGISTERED = True
