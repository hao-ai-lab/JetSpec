"""Compiled read-only tree-VERIFY forward (JetSpec N3, A3-INT).
...
`torch.ops.jetspec.paged_tree_attn` custom_op fusion boundary) -> o-proj + residual,
...
`torch.ops.jetspec.paged_tree_attn` op is registered by importing
`paged_tree_attn_op`, whose triton wrapper is itself lazily imported.
"""
import torch
import torch.nn.functional as F

from jetspec.inference_engine.paged_tree_attn_op import paged_tree_attn  # noqa: F401  (registers jetspec::paged_tree_attn)
