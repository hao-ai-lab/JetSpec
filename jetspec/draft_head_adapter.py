...
draft head (`jetspec.models.draft_head.DFlashDraftModel`). The head owns neither
...
target's `lm_head`. Like the reference, the drafter keeps a draft-head KV cache:
target-hidden context is appended once, speculative block positions are cropped
after each proposal, and the engine owns resetting the cache per generation.
"""
import torch
from transformers import DynamicCache

from jetspec.draft import Drafter, TreeDrafter
...
class _DraftHeadForward:
    ...
    def _forward_head(
        ...
        cached_len = int(self.cache.get_seq_length())
        if cached_len > ctx_len:
            self.reset_cache()
            cached_len = 0
        target_hidden_step = target_hidden[:, cached_len:, :].to(device=self.device, dtype=self.dtype)
        # The head's K/V context is [ctx_tokens ; block positions]; rotary needs
        # absolute positions for only the uncached suffix plus this block.
        ...
        hidden = self.head(
            target_hidden=target_hidden_step,
