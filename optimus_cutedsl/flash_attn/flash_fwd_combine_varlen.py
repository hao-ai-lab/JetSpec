...
from optimus_cutedsl.flash_attn.seqlen_info import SeqlenInfo


class FlashAttentionForwardCombineVarlen:
    def __init__(
        self,
    ...
    def kernel(
        ...
        rows_in_block = cutlass.max(
            Int32(0),
            cutlass.min(
                Int32(self.m_block_size),
                Int32(seqlen - m_block * self.m_block_size),
            ),
        )
        ...
        if rows_in_block > 0:
            ...
            if (const_expr(num_splits_dynamic_ptr is None) or num_splits > 1) and (const_expr(not varlen) or m_block * self.m_block_size < max_idx):
                ...
                thr_max_valid_split = cutlass.max(thr_max_valid_split, Int32(-1))
                thr_max_valid_split = cutlass.min(
                    thr_max_valid_split, Int32(self.max_splits - 1)
                )
                num_splits_limit = cutlass.max(Int32(-1), Int32(num_splits - 1))
                thr_max_valid_split = cutlass.min(thr_max_valid_split, num_splits_limit)
