...
from optimus_cutedsl.flash_attn import ampere_helpers as sm80_utils
from optimus_cutedsl.flash_attn import hopper_helpers as sm90_utils
from optimus_cutedsl.flash_attn import utils
from optimus_cutedsl.flash_attn import copy_utils
from optimus_cutedsl.flash_attn.mask import AttentionMask
from optimus_cutedsl.flash_attn.softmax import Softmax, apply_score_mod_inner
...
from optimus_cutedsl.flash_attn.block_sparse_utils import (
...
from optimus_cutedsl.flash_attn.tile_scheduler import (
...
from optimus_cutedsl.flash_attn.fast_math import FastDivmod


class FlashAttentionForwardPagedBase:
    arch: int = 90
    ...
    def __init__(
        ...
        self.has_attention_gate = has_attention_gate
    ...
    def epilogue(
        ...
        if const_expr(mLSE is not None):
            if const_expr(not seqlen.has_cu_seqlens_q):
                mLSE_cur = (
                    ...
                    else mLSE[None, split_idx, head_idx, batch_idx]
                )
            else:
                offset = seqlen.offset_q if const_expr(not self.pack_gqa) else (0, seqlen.offset_q)
                if const_expr(self.is_split_kv):
                    if const_expr(self.pack_gqa and len(mLSE.shape) == 4):
                        # packed layout: (qpack, total_q, head, split)
                        mLSE_cur = cute.domain_offset(
                            (0, seqlen.offset_q, head_idx, split_idx), mLSE
                        )
                    else:
                        # layout after transpose: (total_q, split, head)
                        mLSE_cur = cute.domain_offset((offset, split_idx, head_idx), mLSE)
                else:
                    if const_expr(self.pack_gqa and len(mLSE.shape) == 3):
                        # packed layout: (qpack, total_q, head)
                        mLSE_cur = cute.domain_offset((0, seqlen.offset_q, head_idx), mLSE)
                    else:
                        # layout after transpose: (total_q, head)
                        mLSE_cur = cute.domain_offset((offset, head_idx), mLSE)
        ...
        if const_expr(not seqlen.has_cu_seqlens_q):
            if const_expr(self.is_split_kv):
                mO_cur = mO[None, None, head_idx, split_idx, batch_idx]
            else:
                mO_cur = mO[None, None, head_idx, batch_idx]
        else:
            mO_batch = seqlen.offset_batch_Q(mO, batch_idx, dim=0)
            if const_expr(self.is_split_kv):
                mO_cur = mO_batch[None, None, head_idx, split_idx]
            else:
                mO_cur = mO_batch[None, None, head_idx]
        # thr_mma = tiled_mma.get_slice(tidx)
...
class FlashAttentionForwardPagedSM90(FlashAttentionForwardPagedBase):
    arch = 90
    ...
    def _paged_make_page_tensor(
        ...
        page = cute.domain_offset((0, 0, head_idx_kv, page_idx), mX)
        layout = cute.make_layout((mX.shape[0], head_dim), stride=(mX.stride[0], mX.stride[1]))
        return cute.make_tensor(page.iterator, layout)

    @cute.jit
    def _paged_issue_tma_load(
        self,
        mX: cute.Tensor,
        sX: cute.Tensor,
        tma_atom_X: cute.CopyAtom,
        pipeline_X: cutlass.pipeline.PipelineAsync,
        producer_state: cutlass.pipeline.PipelineState,
        page_idx: Int32,
        head_idx_kv: Int32,
        head_dim: cutlass.Constexpr[Int32],
    ):
        mX_page = self._paged_make_page_tensor(mX, page_idx, head_idx_kv, head_dim)
        gX = cute.local_tile(mX_page, (self.tile_n, head_dim), (None, 0))
        load_X, _, _ = copy_utils.tma_get_copy_fn(
            tma_atom_X, 0, cute.make_layout(1), gX, sX
        )
        load_X = copy_utils.tma_producer_copy_fn(load_X, pipeline_X)
        load_X(src_idx=0, producer_state=producer_state)
    ...
    def __call__(
        ...
        mV: cute.Tensor,  # (num_pages, page_size, h_k, dv)
        ...
        mAttentionGate: Optional[cute.Tensor] = None,
    ):
        """Configures and launches the flash attention kernel.
        ...
        mPageTable = cute.make_tensor(
            mPageTable.iterator,
            cute.make_layout(mPageTable.shape, stride=new_stride(mPageTable)),
        )
        mGate = None
        if const_expr(self.has_attention_gate):
            mGate = cute.make_tensor(
                mAttentionGate.iterator,
                cute.make_layout(mAttentionGate.shape, stride=new_stride(mAttentionGate)),
            )
        Q_layout_transpose = [0, 2, 1]
        mQ = utils.select(mQ, Q_layout_transpose)
        if const_expr(self.has_attention_gate):
            mGate = utils.select(mGate, Q_layout_transpose)

        num_splits = Int32(1)
        if const_expr(self.is_split_kv):
            num_splits = mO.shape[0]
            O_layout_transpose = [1, 3, 2, 0] if const_expr(mCuSeqlensQ is None) else [1, 3, 2, 0]
        ...
        mO = utils.select(mO, O_layout_transpose)
        ...
        self.use_tma_O = self.arch >= 90 and not self.pack_gqa
        ...
        if const_expr(self.pack_gqa):
            # Pack Q along head -> kv-head groups, keep split dim (if any) untouched.
            shape_Q_packed = (
            ...
            # O / LSE carry split dim at the last axis after the earlier transpose.
            if const_expr(not self.is_split_kv):
                shape_O_packed = (
                ...
                shape_LSE_packed = (
                    ...
                    mK.shape[2],
                    *mLSE.shape[2:],
                ) if const_expr(mLSE is not None) else None
                stride_LSE_packed = (
                ...
                # split layout: mO (total_q, hdimv, head, split)
                shape_O_packed = (
                    (self.qhead_per_kvhead, mO.shape[0]),
                    mO.shape[1],
                    mO.shape[2],
                ...
                if const_expr(mLSE is not None):
                    # mLSE layout after transpose: (head, total_q, split)
                    shape_LSE_packed = (
                    ...
                    shape_LSE_packed = None
                    stride_LSE_packed = None
            ...
            if const_expr(self.has_attention_gate):
                # mGate layout mirrors mO layout along (total_q, 1, head, split)
                shape_gate_packed = (
                    (self.qhead_per_kvhead, mGate.shape[0]),
                    mGate.shape[1],
                    mGate.shape[2],
                    mGate.shape[3],
                )
                stride_gate_packed = (
                    (mGate.stride[2], mGate.stride[0]),
                    mGate.stride[1],
                    mGate.stride[2] * self.qhead_per_kvhead,
                    mGate.stride[3],
                )
                mGate = cute.make_tensor(
                    mGate.iterator, cute.make_layout(shape_gate_packed, stride=stride_gate_packed)
                )
                ...
                if const_expr(not self.use_block_sparsity):
                    ...
                    if const_expr(not self.is_split_kv) or n_block_min < n_block_max:
                        ...
                        pipeline_k.producer_acquire(
                            kv_producer_state, extra_tx_count=extra_tx_count
                        )
                        if const_expr(self.use_tma_Q):
                            load_Q(tma_bar_ptr=pipeline_k.producer_get_barrier(kv_producer_state))
                        page_k = mPageTable[batch_idx, n_block]
                        # Debug: print mapping for first few sequences / blocks
                        # if cute.arch.thread_idx()[0] == 0:
                        #     cute.printf("batch=%d n_block=%d page=%d", batch_idx, n_block, page_k)
                        # cute.printf("n_block:%d", n_block)
                        self._paged_issue_tma_load(
                            mK,
                            sK,
                            tma_atom_K,
                            pipeline_k,
                        ...
                        if const_expr(not self.intra_wg_overlap):
                            ...
                            for i in cutlass.range(n_block_max - 1 - n_block_min, unroll=1):
                                ...
                                page_k_loop = mPageTable[batch_idx, n_block]
                            ...
                            for i in cutlass.range(n_block_max - 1 - n_block_min, unroll=1):
                                ...
                                kv_producer_state.advance()
                                page_k_curr = mPageTable[batch_idx, n_block]
                                pipeline_k.producer_acquire(kv_producer_state)
                                self._paged_issue_tma_load(
                                    mK,
                                    sK,
                                ...
                                page_k_prev = mPageTable[batch_idx, n_block_prev]
                                self._paged_issue_tma_load(
                                    mV,
                                    sV,
                                    tma_atom_V,
                            ...
                            n_block = n_block_min
                            pipeline_v.producer_acquire(kv_producer_state)
                            page_k_final = mPageTable[batch_idx, n_block]
                            self._paged_issue_tma_load(
                                mV,
                                sV,
                                tma_atom_V,
    ...
    def apply_attention_gate(
        ...
        if const_expr(not self.has_attention_gate):
            return row_scale
        gate_vals = cute.make_fragment_like(row_scale, Float32)
        gate_tensor = gate_tile
        for r in cutlass.range(cute.size(row_scale), unroll_full=True):
            row_local = tScS_mn[r][0]
            row_global = m_block * self.tile_m + row_local
            gate_val = Float32(1.0)
            if row_global < seqlen_q:
                gate_val = Float32(gate_tensor[row_local, 0])
            gate_vals[r] = utils.silu(gate_val)
        for r in cutlass.range(cute.size(row_scale), unroll_full=True):
            row_scale[r] = row_scale[r] * gate_vals[r]
        return row_scale
