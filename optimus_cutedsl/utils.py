...
from cutlass import Float32, Int32, Int16, Float8E4M3FN, Float8E5M2
...
# Cache torch->cute conversions to avoid repeated DLPack wrapping on hot paths.
_CUTE_TENSOR_CACHE: dict[int, tuple[weakref.ref, dict[tuple[int, int], cute.Tensor]]] = {}


def _convert_from_dlpack_cached(
    x: Optional[Tensor], *, leading_dim: int, alignment: int = 16, enable_tvm_ffi: bool = True
) -> Optional[cute.Tensor]:
    if x is None:
        return None
    key = id(x)
    entry = _CUTE_TENSOR_CACHE.get(key)
    if entry is not None:
        ref, cached = entry
        if ref() is x:
            cached_tensor = cached.get((leading_dim, alignment))
            if cached_tensor is not None:
                return cached_tensor
        else:
            _CUTE_TENSOR_CACHE.pop(key, None)

    def _cleanup(_):
        _CUTE_TENSOR_CACHE.pop(key, None)

    tensor = from_dlpack(
        x, assumed_align=alignment, enable_tvm_ffi=enable_tvm_ffi
    ).mark_layout_dynamic(leading_dim=leading_dim)
    if entry is not None and entry[0]() is x:
        entry[1][(leading_dim, alignment)] = tensor
    else:
        _CUTE_TENSOR_CACHE[key] = (weakref.ref(x, _cleanup), {(leading_dim, alignment): tensor})
    return tensor
...
def convert_from_dlpack(x, leading_dim, alignment=16, divisibility=1) -> cute.Tensor:
    ...
    logical_dtype = getattr(x, "dtype", None)
    element_override = float8_element_map.get(logical_dtype)
    dlpack_source = x.view(torch.uint8) if element_override is not None else x
    try:
        tensor = cute_from_dlpack(
            dlpack_source, assumed_align=alignment, enable_tvm_ffi=True
        ).mark_layout_dynamic(leading_dim=leading_dim)
    except (ImportError, ModuleNotFoundError):
        # TVM-FFI might be unavailable in some deployments; fall back to the legacy path.
        tensor = cute_from_dlpack(dlpack_source, assumed_align=alignment).mark_layout_dynamic(
